...
export function parseVueSFCFile(filename: string, content: string): SFCDescriptor {
  const { errors, descriptor } = parse(content, {
    filename: filename,
    sourceMap: true,
  })
  if (errors.length) {
...
export async function compileVueSFCFile(filename: string, content: string, descriptor?: SFCDescriptor): Promise<CompiledResult> {
  const compiled = {
    js: '',
    css: '',
    ssr: '',
  ...
  if (!descriptor) {
    descriptor = parseVueSFCFile(filename, content)
  }
  if (
  ...
  compiled.css = doCompileStyle(descriptor, id)
  return compiled
}
